...
- `AddDistributedHttpClient` now calls `SetHandlerLifetime(Timeout.InfiniteTimeSpan)` on each underlying named client, consistent with `IHttpClientBuilder.ConfigureForCloud()`. Previously the `IHttpClientFactory` would recycle the `SocketsHttpHandler` every 2 minutes (the factory default), defeating the `PooledConnectionLifetime` set by `SocketsHttpHandler.ConfigureForCloud()`.
...
[1.1.1]: https://github.com/haiilong/CloudHttp/releases/tag/v1.1.1
